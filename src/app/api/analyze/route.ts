...
const QUOTA_ERROR_KO =
  "Google AI 호출 한도(무료 할당량)에 걸렸어요. 1~2분 뒤 다시 시도하거나, AI Studio에서 요금·한도를 확인해 주세요. Pro가 빡빡하면 .env.local의 GEMINI_MODEL을 gemini-2.5-flash·gemini-2.5-flash-lite 등으로 낮춰 보세요.";

const AUTH_ERROR_KO =
  "API 키가 거절되었거나 이 모델을 쓸 권한이 없어요. GOOGLE_GENERATIVE_AI_API_KEY와 GEMINI_MODEL을 확인해 주세요.";

const MODEL_NOT_FOUND_KO =
  "지정한 모델을 찾을 수 없습니다. AI Studio 문서의 최신 모델명으로 GEMINI_MODEL을 설정해 주세요. (예: gemini-2.5-pro)";
...
const confirmSchema: Schema = {
  ...
  required: ["exists", "location", "venueType", "notice"],
};
...
const responseSchema: Schema = {
  ...
  required: ["exists", "probability", "location", "venueType", "notice"],
};
...
- exists false면: location과 venueType은 반드시 "-", notice에 짧은 한국어 안내를 넣는다.

맛집 확률은 이 단계에서 계산하지 않는다. 반드시 JSON 한 개만 출력한다. 필드: exists, location, venueType, notice. exists true일 때 notice는 빈 문자열.`;
...
exists가 false면 probability는 0, location과 venueType은 \"-\", notice에 짧은 안내 문장을 넣어라.

exists true일 때 location은 가능하면 시·구(또는 시·군)까지 포함한 행정구역으로 적는다.

반드시 JSON 한 개만 출력한다. 필드: exists, probability, location, venueType, notice. exists true일 때 notice는 빈 문자열.`;
...
export async function POST(request: Request) {
  ...
  /* 기본: Pro(품질·일관성 우선). 한도·비용이 부담이면 .env.local → GEMINI_MODEL=gemini-2.5-flash 또는 gemini-2.5-flash-lite */
  const modelName =
    process.env.GEMINI_MODEL?.trim() || "gemini-2.5-pro";

  try {
    if (phase === "confirm") {
      const userText = `검색어(식당 이름만 또는 지역+식당 이름): "${name}"\n위 검색어가 가리키는 음식점이 실제로 있을 법하면 exists true로 두고, location은 반드시 시·구(또는 시·군)까지 포함해 구체적으로, venueType은 업종을 구체적으로 채운다. 그렇지 않으면 exists false와 notice를 채워 JSON만 출력하라.`;
      let parsed: unknown;
      try {
...
이 정보를 모두 반영해 어떤 음식점을 가리키는지 특정한 뒤, 맛집일 확률(0~100)·최종 위치·업종을 JSON으로 출력하라. 실제 음식점으로 특정할 수 없으면 exists false.`
        : `원 검색어: "${name}"
사용자가 아래 위치·업종이 맞다고 확인했다. 이 전제를 우선하되, 검색어·상식과 명백히 모순이면 exists false로 처리할 수 있다.
위치: "${confirmedLocation || "지역 미상"}"
업종: "${confirmedVenueType || "음식점"}"
exists true면 맛집일 확률(0~100)과 location·venueType을 채운다. location·venueType은 확인 내용과 같거나 더 정교하게 다듬어도 된다.`;
