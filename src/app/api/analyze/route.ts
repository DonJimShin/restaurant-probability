...
import { normalizeProbability } from "@/lib/analysis";

const QUOTA_ERROR_KO =
  "Google AI 호출 한도(무료 할당량)에 걸렸어요. 1~2분 뒤 다시 시도하거나, AI Studio에서 요금·한도를 확인해 주세요. 다른 모델은 .env.local에 GEMINI_MODEL=gemini-2.5-flash-lite 등으로 바꿔 보세요.";
...
반드시 JSON 한 개만 출력한다. 필드는 probability(0~100 정수) 하나뿐이다. 설명 문장·한 줄 평·근거 나열은 출력하지 마라.`;
...
export async function POST(request: Request) {
  ...
  /* 1.5 등 구 모델은 단계적 폐기될 수 있어, 현행 문서 기준 Flash 계열 기본값 */
  const modelName =
  ...
  try {
      ...
      contents: [
        {
          ...
          parts: [
            {
              text: `검색어(식당 이름만 또는 지역+식당 이름): "${name}"\n위 가게를 가리키는 것으로 해석하고 맛집 확률만 JSON으로 출력하라.`,
            },
          ],
        },
      ],
    ...
    const normalized = normalizeProbability(parsed);
    if (!normalized) {
      return NextResponse.json(
